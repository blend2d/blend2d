...
LineTo:
          v1 = p1 - _p0;
          if (Geometry::length_sq(v1) < kStrokeLengthEpsilonSq)
            continue;
...
SmoothPolyTo:
          ...
          if (bl_abs(cm) <= kStrokeCollinearityEpsilon) {
            ...
            if (dot > 0.0) {
              ...
              if (t > 0.0 && t < 1.0) {
                poly_pts[0] = Geometry::eval_quad(quad, t);
                poly_pts[1] = p2;
                poly_size = 2;
                goto SmoothPolyTo;
              }
          ...
          if (Geometry::length_sq(v1) < kStrokeLengthEpsilonSq || Geometry::length_sq(v2) < kStrokeLengthEpsilonSq) {
            p1 = p2;
            goto LineTo;
          }
          ...
          if (Geometry::is_cubic_flat(p, kStrokeDegenerateFlatness)) {
            double dot1 = Geometry::dot(p[0] - p[1], p[3] - p[1]);
            double dot2 = Geometry::dot(p[0] - p[2], p[3] - p[2]);

            if (!(dot1 < 0.0) || !(dot2 < 0.0)) {
              // Rotate all points to x-axis.
              const BLPoint r = Geometry::cubic_start_tangent(p);
              ...
              if (t1 > kStrokeCuspTThreshold && t1 < 1.0 - kStrokeCuspTThreshold)
                poly_pts[poly_size++] = Geometry::eval_cubic(p, t1);

              if (t2 > kStrokeCuspTThreshold && t2 < 1.0 - kStrokeCuspTThreshold)
                poly_pts[poly_size++] = Geometry::eval_cubic(p, t2);
            ...
            double tl;
            Geometry::get_cubic_inflection_parameter(p, tCusp, tl);

            if (tl == 0.0 && tCusp > 0.0 && tCusp < 1.0) {
              Geometry::split_cubic(p, p, p + 3);
              cusp = 1;
            }
          }
  ...
  BL_INLINE_IF_NOT_DEBUG BLResult join_line_to(const BLPoint& p1, const BLPoint& n1) noexcept {
    if (_n0 == n1) {
      ...
      BLPoint m = _n0 + n1;
      BLPoint k = m * d2(side) / Geometry::length_sq(m);
  ...
  BL_INLINE_IF_NOT_DEBUG BLResult outer_join(PathAppender& appender, const BLPoint& n1, const BLPoint& w1, const BLPoint& k, double d, double d2, size_t& miter_flag) noexcept {
    ...
    if (Geometry::length_sq(k) <= _miter_limit_sq) {
    ...
    if (_join_type == BL_STROKE_JOIN_MITER_CLIP) {
      ...
      if (b2 > 0)
        b2 = b2 * _miter_limit / Geometry::length(k);
      else
        b2 = _miter_limit;
    ...
    if (_join_type == BL_STROKE_JOIN_ROUND) {
      ...
      if (Geometry::dot(_p0 - pa, _p0 - pb) < 0.0) {
        ...
        BLPoint m = _n0 + n2;
        BLPoint k0 = d2 * m / Geometry::length_sq(m);
        BLPoint q = d * n2;
  ...
  BL_INLINE_IF_NOT_DEBUG BLResult dull_round_join(PathAppender& out, double d, double d2, const BLPoint& w1) noexcept {
    ...
    BLPoint m = _n0 + n2;
    BLPoint k = m * d2 / Geometry::length_sq(m);
    BLPoint q = n2 * d;
  ...
  BL_INLINE_IF_NOT_DEBUG BLResult offset_quad(const BLPoint bez[3]) noexcept {
    double ts[3];
    size_t tn = Geometry::get_quad_offset_cusp_ts(bez, d(), ts);
    ts[tn++] = 1.0;
    ...
    do {
      for (;;) {
        ...
        BLPoint part[3];
        Geometry::split_quad(iter.part, part, iter.part, t);
  ...
  BL_INLINE_IF_NOT_DEBUG void offset_quad_simple(const BLPoint& p0, const BLPoint& p1, const BLPoint& p2) noexcept {
    ...
    BLPoint m = m0 + m2;
    BLPoint k1 = m * d2() / Geometry::length_sq(m);
    BLPoint k2 = m2 * d();
  ...
  BL_INLINE_IF_NOT_DEBUG BLResult offset_cubic(const BLPoint bez[4]) noexcept {
    ...
    return Geometry::approximate_cubic_with_quads(bez, _approx.simplify_tolerance, sink);
  }
