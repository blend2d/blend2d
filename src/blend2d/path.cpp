      ...
      switch (c) {
        ...
        case BL_PATH_CMD_QUAD: {
          ...
          if (!(ctrl.x >= bounding_box.x0 && ctrl.y >= bounding_box.y0 && ctrl.x <= bounding_box.x1 && ctrl.y <= bounding_box.y1)) {
            BLPoint extrema = Geometry::quad_extrema_point(vtx_data - 3);
        ...
        case BL_PATH_CMD_CUBIC: {
          ...
          if (!(ctrl_min.x >= bounding_box.x0 && ctrl_min.y >= bounding_box.y0 && ctrl_max.x <= bounding_box.x1 && ctrl_max.y <= bounding_box.y1)) {
            BLPoint extrema[2];
            Geometry::get_cubic_extrema_points(vtx_data - 4, extrema);
            Geometry::bound(bounding_box, extrema[0]);
            Geometry::bound(bounding_box, extrema[1]);
...
BL_API_IMPL BLResult bl_path_elliptic_arc_to(BLPathCore* self, double rx, double ry, double xAxisRotation, bool large_arc_flag, bool sweep_flag, double x1, double y1) noexcept {
  ...
  double len2 = bl::Geometry::length_sq(v);
  if (len2 < 1.0) {
...
OnLine:
      ...
      case BL_PATH_CMD_QUAD: {
        ...
        if (pt.y >= min_y && pt.y <= max_y) {
          ...
          const BLPoint* spline_end = bl::Geometry::split_quad_to_spline<bl::Geometry::SplitQuadOptions::kYExtrema>(p, spline_data);
          ...
          do {
            ...
            if (pt.y >= min_y && pt.y < max_y) {
              ...
              BLPoint a, b, c;
              bl::Geometry::get_quad_coefficients(spline_ptr, a, b, c);

              // { At^2 + Bt + C } -> { (At + B)t + C }
              if (bl::Math::quad_roots(ti, a.y, b.y, c.y - pt.y, bl::Math::kAfter0, bl::Math::kBefore1) >= 1)
      ...
      case BL_PATH_CMD_CUBIC: {
        ...
        if (pt.y >= min_y && pt.y <= max_y) {
          ...
          const BLPoint* spline_ptr = p;
          const BLPoint* spline_end = bl::Geometry::split_cubic_to_spline<bl::Geometry::SplitCubicOptions::kYExtremas>(p, spline_data);
          ...
          do {
            ...
            if (pt.y >= min_y && pt.y < max_y) {
              ...
              BLPoint a, b, c, d;
              bl::Geometry::get_cubic_coefficients(spline_ptr, a, b, c, d);

              // { At^3 + Bt^2 + Ct + D } -> { ((At + B)t + C)t + D }
              if (bl::Math::cubic_roots(ti, a.y, b.y, c.y, d.y - pt.y, bl::Math::kAfter0, bl::Math::kBefore1) >= 1)
