...
static TestVVFunc create_func_vv(JitContext& ctx, SimdWidth simdWidth, OpcodeVV op, uint32_t variation = 0) noexcept {
  ...
  // sure that we can test this function (that the low lart is actually zeroed and doesn't contain a garbage).
  pc.v_zero_i(dstVec);

  if (variation <= 1) {
    if (variation == 1 && (op == OpcodeVV::kBroadcastU8 ||
...
template<typename T>
static bool vec_eq(const T* observed, const T* expected, uint32_t count) noexcept {
...
template<typename T> struct vec_op_imul : public op_each_vvv<T, vec_op_imul<T>> {
...
template<uint32_t kW>
static BL_NOINLINE void test_vecop_vv_failed(OpcodeVV op, const VecOverlay<kW>& arg0, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, const char* assembly) noexcept {
...
template<uint32_t kW>
static BL_NOINLINE void test_vecop_vvi_failed(OpcodeVVI op, const VecOverlay<kW>& arg0, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, uint32_t imm, const char* assembly) noexcept {
...
template<uint32_t kW>
static BL_NOINLINE void test_vecop_vvv_failed(OpcodeVVV op, const VecOverlay<kW>& arg0, const VecOverlay<kW>& arg1, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, const char* assembly) noexcept {
...
template<uint32_t kW>
static BL_NOINLINE void test_vecop_vvvi_failed(OpcodeVVVI op, const VecOverlay<kW>& arg0, const VecOverlay<kW>& arg1, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, uint32_t imm, const char* assembly) noexcept {
...
template<uint32_t kW>
static BL_NOINLINE void test_vecop_vvvv_failed(OpcodeVVVV op, const VecOverlay<kW>& arg0, const VecOverlay<kW>& arg1, const VecOverlay<kW>& arg2, const VecOverlay<kW>& observed, const VecOverlay<kW>& expected, const char* assembly) noexcept {
...
template<SimdWidth kSimdWidth, OpcodeVV kOp, typename GenericOp, typename Constraint>
static BL_NOINLINE void test_vecop_vv_constraint(JitContext& ctx, uint32_t variation = 0) noexcept {
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected))
      test_vecop_vv_failed(kOp, a, observed, expected, ctx.logger.data());
  }

  ctx.rt.release(compiledApply);
}

template<SimdWidth kSimdWidth, OpcodeVV kOp, typename GenericOp>
static void test_vecop_vv(JitContext& ctx, uint32_t variation = 0) noexcept {
  return test_vecop_vv_constraint<kSimdWidth, kOp, GenericOp, ConstraintNone>(ctx, variation);
}

// bl::Pipeline::JIT - Tests - SIMD - Integer Operations - VVI
// ===========================================================

template<SimdWidth kSimdWidth, OpcodeVVI kOp, typename GenericOp, typename Constraint>
static BL_NOINLINE void test_vecop_vvi_constraint(JitContext& ctx, uint32_t imm) noexcept {
  constexpr uint32_t kW = vecWidthFromSimdWidth(kSimdWidth);
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected))
      test_vecop_vvi_failed(kOp, a, observed, expected, imm, ctx.logger.data());
  }

  ctx.rt.release(compiledApply);
}

template<SimdWidth kSimdWidth, OpcodeVVI kOp, typename GenericOp>
static void test_vecop_vvi(JitContext& ctx, uint32_t imm) noexcept {
  return test_vecop_vvi_constraint<kSimdWidth, kOp, GenericOp, ConstraintNone>(ctx, imm);
}

// bl::Pipeline::JIT - Tests - SIMD - Integer Operations - VVV
// ===========================================================

template<SimdWidth kSimdWidth, OpcodeVVV kOp, typename GenericOp, typename Constraint>
static BL_NOINLINE void test_vecop_vvv_constraint(JitContext& ctx) noexcept {
  constexpr uint32_t kW = vecWidthFromSimdWidth(kSimdWidth);
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected))
      test_vecop_vvv_failed(kOp, a, b, observed, expected, ctx.logger.data());
  }

  ctx.rt.release(compiledApply);
}

template<SimdWidth kSimdWidth, OpcodeVVV kOp, typename GenericOp>
static void test_vecop_vvv(JitContext& ctx) noexcept {
  return test_vecop_vvv_constraint<kSimdWidth, kOp, GenericOp, ConstraintNone>(ctx);
}

// bl::Pipeline::JIT - Tests - SIMD - Integer Operations - VVVI
// ============================================================

template<SimdWidth kSimdWidth, OpcodeVVVI kOp, typename GenericOp, typename Constraint>
static BL_NOINLINE void test_vecop_vvvi_constraint(JitContext& ctx, uint32_t imm) noexcept {
  constexpr uint32_t kW = vecWidthFromSimdWidth(kSimdWidth);
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected))
      test_vecop_vvvi_failed(kOp, a, b, observed, expected, imm, ctx.logger.data());
  }

  ctx.rt.release(compiledApply);
}

template<SimdWidth kSimdWidth, OpcodeVVVI kOp, typename GenericOp>
static void test_vecop_vvvi(JitContext& ctx, uint32_t imm) noexcept {
  return test_vecop_vvvi_constraint<kSimdWidth, kOp, GenericOp, ConstraintNone>(ctx, imm);
}
...
static BL_NOINLINE void test_vecop_vvvv_constraint(JitContext& ctx) noexcept {
  constexpr uint32_t kW = vecWidthFromSimdWidth(kSimdWidth);
  ...
  for (uint32_t iter = 0; iter < kTestIterCount; iter++) {
    ...
    if (!vec_eq(observed, expected))
      test_vecop_vvvv_failed(kOp, a, b, c, observed, expected);
  }
}

template<SimdWidth kSimdWidth, typename T, OpcodeVVVV kOp, typename GenericOp>
static void test_vecop_vvvv(JitContext& ctx) noexcept {
  return test_vecop_vvvv_constraint<kSimdWidth, T, kOp, GenericOp, ConstraintNone>(ctx);
}
...
static BL_NOINLINE void test_simd_ops(JitContext& ctx) noexcept {
#if defined(BL_JIT_ARCH_X86)
  constexpr ScalarOpBehavior kScalarOpBehavior = ScalarOpBehavior::kPreservingVec128;
#else
  constexpr ScalarOpBehavior kScalarOpBehavior = ScalarOpBehavior::kZeroing;
#endif

  INFO("  Testing mov");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kMov, vec_op_mov>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kMovU64, vec_op_mov_u64>(ctx);
  }
  ...
  INFO("  Testing broadcast");
  {
    ...
    for (uint32_t variation = 0; variation < 3; variation++) {
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastU8Z, vec_op_broadcast_u8>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastU16Z, vec_op_broadcast_u16>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastU8, vec_op_broadcast_u8>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastU16, vec_op_broadcast_u16>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastU32, vec_op_broadcast_u32>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastU64, vec_op_broadcast_u64>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastF32, vec_op_broadcast_u32>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastF64, vec_op_broadcast_u64>(ctx, variation);

      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV128_U32, vec_op_broadcast_u128>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV128_U64, vec_op_broadcast_u128>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV128_F32, vec_op_broadcast_u128>(ctx, variation);
      test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV128_F64, vec_op_broadcast_u128>(ctx, variation);

      if (kSimdWidth > SimdWidth::k256) {
        test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV256_U32, vec_op_broadcast_u256>(ctx, variation);
        test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV256_U64, vec_op_broadcast_u256>(ctx, variation);
        test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV256_F32, vec_op_broadcast_u256>(ctx, variation);
        test_vecop_vv<kSimdWidth, OpcodeVV::kBroadcastV256_F64, vec_op_broadcast_u256>(ctx, variation);
      }
    }
  }
  ...
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kAbsI8, vec_op_abs<int8_t>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kAbsI16, vec_op_abs<int16_t>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kAbsI32, vec_op_abs<int32_t>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kAbsI64, vec_op_abs<int64_t>>(ctx);
  }

  INFO("  Testing not (int)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kNotU32, vec_op_not<uint32_t>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kNotU64, vec_op_not<uint64_t>>(ctx);
  }

  INFO("  Testing cvt (int)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI8LoToI16, vec_op_cvt_i8_lo_to_i16>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI8HiToI16, vec_op_cvt_i8_hi_to_i16>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU8LoToU16, vec_op_cvt_u8_lo_to_u16>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU8HiToU16, vec_op_cvt_u8_hi_to_u16>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI8ToI32, vec_op_cvt_i8_to_i32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU8ToU32, vec_op_cvt_u8_to_u32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI16LoToI32, vec_op_cvt_i16_lo_to_i32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI16HiToI32, vec_op_cvt_i16_hi_to_i32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU16LoToU32, vec_op_cvt_u16_lo_to_u32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU16HiToU32, vec_op_cvt_u16_hi_to_u32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI32LoToI64, vec_op_cvt_i32_lo_to_i64>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI32HiToI64, vec_op_cvt_i32_hi_to_i64>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU32LoToU64, vec_op_cvt_u32_lo_to_u64>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtU32HiToU64, vec_op_cvt_u32_hi_to_u64>(ctx);
  }

  INFO("  Testing abs (float)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kAbsF32, vec_op_fabs<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kAbsF64, vec_op_fabs<double>>(ctx);
  }

  INFO("  Testing not (float)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kNotF32, vec_op_not<uint32_t>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kNotF64, vec_op_not<uint64_t>>(ctx);
  }

  INFO("  Testing rounding (float)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kTruncF32S, scalar_op_trunc<kScalarOpBehavior, float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kTruncF64S, scalar_op_trunc<kScalarOpBehavior, double>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kFloorF32S, scalar_op_floor<kScalarOpBehavior, float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kFloorF64S, scalar_op_floor<kScalarOpBehavior, double>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCeilF32S, scalar_op_ceil<kScalarOpBehavior, float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCeilF64S, scalar_op_ceil<kScalarOpBehavior, double>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kRoundF32S, scalar_op_round<kScalarOpBehavior, float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kRoundF64S, scalar_op_round<kScalarOpBehavior, double>>(ctx);

    test_vecop_vv<kSimdWidth, OpcodeVV::kTruncF32, vec_op_trunc<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kTruncF64, vec_op_trunc<double>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kFloorF32, vec_op_floor<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kFloorF64, vec_op_floor<double>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCeilF32, vec_op_ceil<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCeilF64, vec_op_ceil<double>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kRoundF32, vec_op_round<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kRoundF64, vec_op_round<double>>(ctx);
  }

  INFO("  Testing rcp (float)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kRcpF32, vec_op_rcp<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kRcpF64, vec_op_rcp<double>>(ctx);
  }

  INFO("  Testing sqrt (float)");
  {
    test_vecop_vv<kSimdWidth, OpcodeVV::kSqrtF32S, scalar_op_sqrt<kScalarOpBehavior, float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kSqrtF64S, scalar_op_sqrt<kScalarOpBehavior, double>>(ctx);

    test_vecop_vv<kSimdWidth, OpcodeVV::kSqrtF32, vec_op_sqrt<float>>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kSqrtF64, vec_op_sqrt<double>>(ctx);
  }

  INFO("  Testing cvt (float)");
  {
    // TODO: Re-enable when the content of the remaining part of the register is formalized.
    // test_vecop_vv<kSimdWidth, OpcodeVV::kCvtF32ToF64S, scalar_op_cvt_f32_to_f64>(ctx);
    // test_vecop_vv<kSimdWidth, OpcodeVV::kCvtF64ToF32S, scalar_op_cvt_f64_to_f32>(ctx);

    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI32ToF32, vec_op_cvt_i32_to_f32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtF32LoToF64, vec_op_cvt_f32_lo_to_f64>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtF32HiToF64, vec_op_cvt_f32_hi_to_f64>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtF64ToF32Lo, vec_op_cvt_f64_to_f32_lo>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtF64ToF32Hi, vec_op_cvt_f64_to_f32_hi>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI32LoToF64, vec_op_cvt_i32_lo_to_f64>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtI32HiToF64, vec_op_cvt_i32_hi_to_f64>(ctx);

    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtTruncF32ToI32, vec_op_cvt_trunc_f32_to_i32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtTruncF64ToI32Lo, vec_op_cvt_trunc_f64_to_i32_lo>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtTruncF64ToI32Hi, vec_op_cvt_trunc_f64_to_i32_hi>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtRoundF32ToI32, vec_op_cvt_round_f32_to_i32>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtRoundF64ToI32Lo, vec_op_cvt_round_f64_to_i32_lo>(ctx);
    test_vecop_vv<kSimdWidth, OpcodeVV::kCvtRoundF64ToI32Hi, vec_op_cvt_round_f64_to_i32_hi>(ctx);
  }
...
/*
    for (uint32_t i = 1; i < 8; i++) {
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSllU8 , vec_op_slli<uint8_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSrlU8 , vec_op_srli<uint8_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSraI8 , vec_op_srai<int8_t>>(ctx, i);
    }
*/
    for (uint32_t i = 1; i < 16; i++) {
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSllU16, vec_op_slli<uint16_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSrlU16, vec_op_srli<uint16_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSraI16, vec_op_srai<int16_t>>(ctx, i);
    }

    for (uint32_t i = 1; i < 32; i++) {
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSllU32, vec_op_slli<uint32_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSrlU32, vec_op_srli<uint32_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSraI32, vec_op_srai<int32_t>>(ctx, i);
    }

    for (uint32_t i = 1; i < 64; i++) {
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSllU64, vec_op_slli<uint64_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSrlU64, vec_op_srli<uint64_t>>(ctx, i);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSraI64, vec_op_srai<int64_t>>(ctx, i);
    }
  }
  ...
  {
    for (uint32_t i = 1; i < 16; i++) {
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSllbU128, vec_op_sllb_u128>(ctx, i);
    }
  }
  ...
  {
    for (uint32_t i = 0; i < 16; i++) {
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSrlbU128, vec_op_srlb_u128>(ctx, i);
    }
  }
  ...
  INFO("  Testing swizzle_[lo|hi]_u16x4");
  {
    for (uint32_t i = 0; i < 256; i++) {
      ...
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleLoU16x4, vec_op_swizzle_lo_u16x4>(ctx, imm);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleHiU16x4, vec_op_swizzle_hi_u16x4>(ctx, imm);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleU16x4, vec_op_swizzle_u16>(ctx, imm);
    }
  }
  ...
  INFO("  Testing swizzle_u32x4");
  {
    for (uint32_t i = 0; i < 256; i++) {
      ...
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleU32x4, vec_op_swizzle_u32x4>(ctx, imm);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleF32x4, vec_op_swizzle_u32x4>(ctx, imm);
    }
  }
  ...
  INFO("  Testing swizzle_u64x2");
  {
    for (uint32_t i = 0; i < 4; i++) {
      ...
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleU64x2, vec_op_swizzle_u64x2>(ctx, imm);
      test_vecop_vvi<kSimdWidth, OpcodeVVI::kSwizzleF64x2, vec_op_swizzle_u64x2>(ctx, imm);
    }
  }

  INFO("  Testing logical (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndU32, vec_op_and<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndU64, vec_op_and<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kOrU32, vec_op_or<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kOrU64, vec_op_or<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kXorU32, vec_op_xor<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kXorU64, vec_op_xor<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndnU32, vec_op_andn<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndnU64, vec_op_andn<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kBicU32, vec_op_bic<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kBicU64, vec_op_bic<uint64_t>>(ctx);
  }

  INFO("  Testing add / adds (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddU8, vec_op_add<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddU16, vec_op_add<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddU32, vec_op_add<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddU64, vec_op_add<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddsI8, vec_op_adds<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddsI16, vec_op_adds<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddsU8, vec_op_adds<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddsU16, vec_op_adds<uint16_t>>(ctx);
  }

  INFO("  Testing sub / subs (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubU8, vec_op_sub<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubU16, vec_op_sub<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubU32, vec_op_sub<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubU64, vec_op_sub<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubsI8, vec_op_subs<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubsI16, vec_op_subs<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubsU8, vec_op_subs<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubsU16, vec_op_subs<uint16_t>>(ctx);
  }

  INFO("  Testing mul (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulU16, vec_op_imul<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulU32, vec_op_imul<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulU64, vec_op_imul<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulhI16, vec_op_mulhi<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulhU16, vec_op_mulhu<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulU64_LoU32, vec_op_mul_u64_lo_u32>(ctx);
  }

  INFO("  Testing mhadd (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMHAddI16_I32, vec_op_mhadd_i16_i32>(ctx);
  }

  INFO("  Testing min / max (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinI8, vec_op_min<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinI16, vec_op_min<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinI32, vec_op_min<int32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinI64, vec_op_min<int64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinU8, vec_op_min<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinU16, vec_op_min<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinU32, vec_op_min<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinU64, vec_op_min<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxI8, vec_op_max<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxI16, vec_op_max<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxI32, vec_op_max<int32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxI64, vec_op_max<int64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxU8, vec_op_max<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxU16, vec_op_max<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxU32, vec_op_max<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxU64, vec_op_max<uint64_t>>(ctx);
  }

  INFO("  Testing cmp (int)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpEqU8, vec_op_cmp_eq<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpEqU16, vec_op_cmp_eq<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpEqU32, vec_op_cmp_eq<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpEqU64, vec_op_cmp_eq<uint64_t>>(ctx);
/*
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpNeU8, vec_op_cmp_ne<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpNeU16, vec_op_cmp_ne<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpNeU32, vec_op_cmp_ne<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpNeU64, vec_op_cmp_ne<uint64_t>>(ctx);
*/
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtI8, vec_op_cmp_gt<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtI16, vec_op_cmp_gt<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtI32, vec_op_cmp_gt<int32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtI64, vec_op_cmp_gt<int64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtU8, vec_op_cmp_gt<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtU16, vec_op_cmp_gt<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtU32, vec_op_cmp_gt<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtU64, vec_op_cmp_gt<uint64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeI8, vec_op_cmp_ge<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeI16, vec_op_cmp_ge<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeI32, vec_op_cmp_ge<int32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeI64, vec_op_cmp_ge<int64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeU8, vec_op_cmp_ge<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeU16, vec_op_cmp_ge<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeU32, vec_op_cmp_ge<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeU64, vec_op_cmp_ge<uint64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtI8, vec_op_cmp_lt<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtI16, vec_op_cmp_lt<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtI32, vec_op_cmp_lt<int32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtI64, vec_op_cmp_lt<int64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtU8, vec_op_cmp_lt<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtU16, vec_op_cmp_lt<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtU32, vec_op_cmp_lt<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtU64, vec_op_cmp_lt<uint64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeI8, vec_op_cmp_le<int8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeI16, vec_op_cmp_le<int16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeI32, vec_op_cmp_le<int32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeI64, vec_op_cmp_le<int64_t>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeU8, vec_op_cmp_le<uint8_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeU16, vec_op_cmp_le<uint16_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeU32, vec_op_cmp_le<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeU64, vec_op_cmp_le<uint64_t>>(ctx);
  }

  INFO("  Testing logical (float)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndF32, vec_op_and<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndF64, vec_op_and<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kOrF32, vec_op_or<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kOrF64, vec_op_or<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kXorF32, vec_op_xor<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kXorF64, vec_op_xor<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndnF32, vec_op_andn<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAndnF64, vec_op_andn<uint64_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kBicF32, vec_op_bic<uint32_t>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kBicF64, vec_op_bic<uint64_t>>(ctx);
  }

  INFO("  Testing arithmetic (float)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddF32S, scalar_op_fadd<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddF64S, scalar_op_fadd<kScalarOpBehavior, double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubF32S, scalar_op_fsub<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubF64S, scalar_op_fsub<kScalarOpBehavior, double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulF32S, scalar_op_fmul<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulF64S, scalar_op_fmul<kScalarOpBehavior, double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kDivF32S, scalar_op_fdiv<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kDivF64S, scalar_op_fdiv<kScalarOpBehavior, double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddF32, vec_op_fadd<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kAddF64, vec_op_fadd<double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubF32, vec_op_fsub<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kSubF64, vec_op_fsub<double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulF32, vec_op_fmul<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMulF64, vec_op_fmul<double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kDivF32, vec_op_fdiv<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kDivF64, vec_op_fdiv<double>>(ctx);
  }

  INFO("  Testing min / max (float)");
  {
#if defined(BL_JIT_ARCH_X86)
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF32S, scalar_op_fmin_ternary<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF64S, scalar_op_fmin_ternary<kScalarOpBehavior, double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF32S, scalar_op_fmax_ternary<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF64S, scalar_op_fmax_ternary<kScalarOpBehavior, double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF32, vec_op_fmin_ternary<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF64, vec_op_fmin_ternary<double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF32, vec_op_fmax_ternary<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF64, vec_op_fmax_ternary<double>>(ctx);
#endif

#if defined(BL_JIT_ARCH_A64)
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF32S, scalar_op_fmin_finite<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF64S, scalar_op_fmin_finite<kScalarOpBehavior, double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF32S, scalar_op_fmax_finite<kScalarOpBehavior, float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF64S, scalar_op_fmax_finite<kScalarOpBehavior, double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF32, vec_op_fmin_finite<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMinF64, vec_op_fmin_finite<double>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF32, vec_op_fmax_finite<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kMaxF64, vec_op_fmax_finite<double>>(ctx);
#endif
  }

  INFO("  Testing cmp (float)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpEqF32, vec_op_fcmpo_eq<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpEqF64, vec_op_fcmpo_eq<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpNeF32, vec_op_fcmpu_ne<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpNeF64, vec_op_fcmpu_ne<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtF32, vec_op_fcmpo_gt<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGtF64, vec_op_fcmpo_gt<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeF32, vec_op_fcmpo_ge<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpGeF64, vec_op_fcmpo_ge<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtF32, vec_op_fcmpo_lt<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLtF64, vec_op_fcmpo_lt<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeF32, vec_op_fcmpo_le<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpLeF64, vec_op_fcmpo_le<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpOrdF32, vec_op_fcmp_ord<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpOrdF64, vec_op_fcmp_ord<double>>(ctx);

    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpUnordF32, vec_op_fcmp_unord<float>>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCmpUnordF64, vec_op_fcmp_unord<double>>(ctx);
  }

  INFO("  Testing hadd (float)");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kHAddF64, vec_op_hadd_f64>(ctx);
  }

  INFO("  Testing combine");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCombineLoHiU64, vec_op_combine_lo_hi_u64>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCombineLoHiF64, vec_op_combine_lo_hi_u64>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCombineHiLoU64, vec_op_combine_hi_lo_u64>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kCombineHiLoF64, vec_op_combine_hi_lo_u64>(ctx);
  }

  INFO("  Testing interleave");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveLoU8, vec_op_interleave_lo_u8>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveHiU8, vec_op_interleave_hi_u8>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveLoU16, vec_op_interleave_lo_u16>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveHiU16, vec_op_interleave_hi_u16>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveLoU32, vec_op_interleave_lo_u32>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveHiU32, vec_op_interleave_hi_u32>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveLoU64, vec_op_interleave_lo_u64>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kInterleaveHiU64, vec_op_interleave_hi_u64>(ctx);
  }

  INFO("  Testing packs");
  {
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kPacksI16_I8, vec_op_packs_i16_i8>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kPacksI16_U8, vec_op_packs_i16_u8>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kPacksI32_I16, vec_op_packs_i32_i16>(ctx);
    test_vecop_vvv<kSimdWidth, OpcodeVVV::kPacksI32_U16, vec_op_packs_i32_u16>(ctx);
  }

  INFO("  Testing alignr_u128");
  {
    for (uint32_t i = 1; i < 16; i++)
      test_vecop_vvvi<kSimdWidth, OpcodeVVVI::kAlignr_U128, vec_op_alignr_u128>(ctx, i);
  }
  ...
  INFO("  Testing interleave_shuffle");
  {
    for (uint32_t i = 0; i < 256; i++) {
      ...
      test_vecop_vvvi<kSimdWidth, OpcodeVVVI::kInterleaveShuffleU32x4, vec_op_interleave_shuffle_u32x4>(ctx, imm);
      test_vecop_vvvi<kSimdWidth, OpcodeVVVI::kInterleaveShuffleF32x4, vec_op_interleave_shuffle_u32x4>(ctx, imm);
    }

    for (uint32_t i = 0; i < 4; i++) {
      uint32_t imm = swizzle((i >> 1) & 1, i & 1).value;

      test_vecop_vvvi<kSimdWidth, OpcodeVVVI::kInterleaveShuffleU64x2, vec_op_interleave_shuffle_u64x2>(ctx, imm);
      test_vecop_vvvi<kSimdWidth, OpcodeVVVI::kInterleaveShuffleF64x2, vec_op_interleave_shuffle_u64x2>(ctx, imm);
    }
  }
}
