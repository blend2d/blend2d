...
CompOpPart::CompOpPart(PipeCompiler* pc, CompOpExt compOp, FetchPart* dstPart, FetchPart* srcPart) noexcept
  ...
  // TODO: AArch64 JIT: Every composition mode should use packed in the future...
...
void CompOpPart::nextPartialPixel() noexcept {
  ...
  switch (pixelType()) {
    case PixelType::kA8: {
      ...
      FetchUtils::shiftOrRotate(pc, pix, pix, 2);
      break;
    }

    case PixelType::kRGBA32: {
      const Vec& pix = _partialPixel.pc[0];
      FetchUtils::shiftOrRotate(pc, pix, pix, 4);
      break;
    }
...
void CompOpPart::cMaskProcStoreAdvance(const Gp& dPtr, PixelCount n, Alignment alignment, PixelPredicate& predicate) noexcept {
  ...
  switch (pixelType()) {
    case PixelType::kA8: {
      ...
      FetchUtils::x_store_pixel_advance(pc, dPtr, dPix, n, 1, alignment, predicate);
      break;
    }

    case PixelType::kRGBA32: {
      cMaskProcRGBA32Vec(dPix, n, PixelFlags::kImmutable, predicate);
      FetchUtils::x_store_pixel_advance(pc, dPtr, dPix, n, 4, alignment, predicate);
      break;
    }
  ...
  switch (pixelType()) {
    case PixelType::kA8: {
      if (n == 1u) {
        ...
        FetchUtils::x_store_pixel_advance(pc, dPtr, dPix, n, 1, Alignment(1), pc->emptyPredicate());
      }
      else {
        ...
        VecArray vm;
        FetchUtils::fetchMaskA8AndAdvance(pc, vm, n, pixelType(), coverageFormat(), mPtr, ga.as<Vec>());
...
void CompOpPart::cMaskProcA8Gp(Pixel& out, PixelFlags flags) noexcept {
  ...
  if (srcPart()->isSolid()) {
    ...
    if (isSrcCopy()) {
      ...
      FetchUtils::x_satisfy_pixel(pc, out, flags);
      return;
    }
...
void CompOpPart::vMaskProcA8Vec(Pixel& out, PixelCount n, PixelFlags flags, const VecArray& vm_, PixelCoverageFlags coverageFlags, PixelPredicate& predicate) noexcept {
  SimdWidth vw = pc->simdWidthOf(DataWidth::k16, n);
  uint32_t kFullN = pc->regCountOf(DataWidth::k16, n);
  ...
  if (srcPart()->isSolid()) {
    ...
    // ------------------------------------
      ...
      if (!hasMask) {
        ...
        pc->v_swizzlev_u8(o.py, o.py, pc->simdVecConst(&pc->ct.pshufb_3xxx2xxx1xxx0xxx_to_3333222211110000, o.py));
      }
#else
      if (!hasMask) {
...
void CompOpPart::cMaskProcRGBA32Vec(Pixel& out, PixelCount n, PixelFlags flags, PixelPredicate& predicate) noexcept {
  ...
  SimdWidth vw = pc->simdWidthOf(DataWidth::k64, n);
...
void CompOpPart::vMaskProcRGBA32Vec(Pixel& out, PixelCount n, PixelFlags flags, const VecArray& vm_, PixelCoverageFlags coverageFlags, PixelPredicate& predicate) noexcept {
  ...
  if (isSrcOver()) {
    ...
    if (!hasMask) {
      ...
      pc->v_swizzlev_u8(yv_half, xv_half, pc->simdVecConst(&pc->ct.pshufb_3xxx2xxx1xxx0xxx_to_3333222211110000, yv_half));
  ...
  if (isDstOut()) {
    ...
    FetchUtils::x_satisfy_pixel(pc, out, flags);
    if (!useDa)
      FetchUtils::x_fill_pixel_alpha(pc, out);
    return;
  }
