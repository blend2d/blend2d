...
static BL_NOINLINE void emitMemFillSequence(PipeCompiler* pc, Mem dPtr, Vec sVec, uint32_t numBytes, MemOpMode mode) noexcept {
  uint32_t n = numBytes;
  ...
  if (mode == MemOpMode::kAdvancePointers) {
  ...
  bool postIndex = (mode == MemOpMode::kAdvancePointers) && !dPtr.hasOffset();
  if (postIndex) {
  ...
  if (mode == MemOpMode::kAdvancePointers && !postIndex) {
    Gp dPtrBase = dPtr.baseReg().as<Gp>();
    pc->add(dPtrBase, dPtrBase, numBytes);
  }
#else
...
void inlineFillSpanLoop(
  ...
  if (granularityInBytes >= 16u) {
...

    {
      ...
      pc->bind(L_MainIter);
      emitMemFillSequence(pc, mem_ptr(dst), src, mainLoopSize, MemOpMode::kAdvancePointers);
    ...
    if (mainLoopSize * 2 > granularityInBytes) {
      ...
      pc->bind(L_TailIter);
      emitMemFillSequence(pc, mem_ptr(dst), src, granularityInBytes, MemOpMode::kAdvancePointers);
  ...
  if (granularityInBytes == 4u) {
    ...
    if (vecSize >= 32u) {
      ...
      if (pc->hasMaskedAccessOf(4) && pc->hasOptFlag(PipeOptFlags::kFastStoreWithMask)) {
        ...
        pc->bind(L_MainIter);
        emitMemFillSequence(pc, mem_ptr(dst), src, vecSize * 4u, MemOpMode::kAdvancePointers);
        ...
        pc->bind(L_TailIter);
        emitMemFillSequence(pc, mem_ptr(dst), src, vecSize, MemOpMode::kAdvancePointers);
        ...
        pc->x_ensure_predicate_32(predicate, vecSize / 4u);
        pc->v_store_predicated_v32(mem_ptr(dst), predicate, src);
...
#endif // BL_JIT_ARCH_X86
      {
        ...
        pc->bind(L_LargeIter);
        emitMemFillSequence(pc, mem_ptr(dst), src, vecSize * 4, MemOpMode::kAdvancePointers);
...

      {
        ...
        pc->bind(L_MainIter);
        emitMemFillSequence(pc, mem_ptr(dst), src.v128(), mainLoopSize, MemOpMode::kAdvancePointers);
        pc->j(L_MainIter, sub_nc(i, mainStepInItems));
...
static BL_NOINLINE void emitMemCopySequence(
  ...
  Mem sPtr, bool srcAligned, uint32_t numBytes, const Vec& fillMask, MemOpMode mode) noexcept {
  ...
  bool postIndex = (mode == MemOpMode::kAdvancePointers) && !dPtr.hasOffset() && !sPtr.hasOffset();
  if (postIndex) {
    dPtr.setOffsetMode(a64::OffsetMode::kPostIndex);
...
void inlineCopySpanLoop(
  ...
  if (format == FormatExt::kXRGB32)
    fillMask = pc->simdVecConst(&commonTable.i_FF000000FF000000, t0);
  ...
  if (granularityInBytes >= 16u) {
...

    {
      ...
      pc->bind(L_MainIter);
      emitMemCopySequence(pc, mem_ptr(dst), false, mem_ptr(src), false, mainLoopSize, fillMask, MemOpMode::kAdvancePointers);
      pc->j(L_MainIter, sub_nc(i, mainStepInItems));
    ...
    if (mainLoopSize * 2 > granularityInBytes) {
      ...
      pc->bind(L_TailIter);
      emitMemCopySequence(pc, mem_ptr(dst), false, mem_ptr(src), false, granularityInBytes, fillMask, MemOpMode::kAdvancePointers);
      pc->j(L_TailIter, sub_nz(i, itemGranularity));
    }
    else if (mainLoopSize * 2 == granularityInBytes) {
      emitMemCopySequence(pc, mem_ptr(dst), false, mem_ptr(src), false, granularityInBytes, fillMask, MemOpMode::kAdvancePointers);
  ...
  if (granularityInBytes == 4u) {
...

    {
      ...
      pc->bind(L_MainIter);
      emitMemCopySequence(pc, mem_ptr(dst), true, mem_ptr(src), false, mainLoopSize, fillMask, MemOpMode::kAdvancePointers);
      pc->j(L_MainIter, sub_nc(i, mainStepInItems));
    ...
    if (mainLoopSize > 32) {
      ...
      pc->bind(L_TailIter);
      emitMemCopySequence(pc, mem_ptr(dst), true, mem_ptr(src), false, 16, fillMask, MemOpMode::kAdvancePointers);
      pc->j(L_TailIter, sub_nc(i, tailStepInItems));
  ...
  if (granularityInBytes == 1) {
...

    {
      ...
      pc->add(dst, dst, i.cloneAs(dst));
      emitMemCopySequence(pc, mem_ptr(dst, -16), false, mem_ptr(src, -16), false, 16, fillMask, MemOpMode::kKeepPointers);
    }
