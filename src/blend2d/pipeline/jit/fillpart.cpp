...
void FillAnalyticPart::compile(const PipeFunction& fn) noexcept {
  ...
  // ---------------------------------
    ...
    if (pixelType == PixelType::kRGBA32) {
      ...
      FetchUtils::shiftOrRotate(pc, m[0], m[0], 2);          //   m0[15:0] = [??, m[15:2]]
    }
    else if (pixelType == PixelType::kA8) {
      pc->bind(L_VLoop_Step);                                // L_VLoop_Step:
  ...
  if (maxPixels >= 4u) {
    ...
    if (pixelType == PixelType::kRGBA32) {
      ...
      if (coverageFormat == PixelCoverageFormat::kPacked) {
        FetchUtils::shiftOrRotate(pc, m[0], m[0], 4);        //   m0[15:0] = [????, m[15:4]]
      }
      else {
#if defined(BL_JIT_ARCH_X86)
        if (hasV256Mask)
      ...
      pc->add(dstPtr, dstPtr, dstBpp);                       //   dstPtr += dstBpp;
      if (coverageFormat == PixelCoverageFormat::kPacked)
        FetchUtils::shiftOrRotate(pc, m128, m128, 1);        //   m0[15:0] = [?, m[15:1]]
      else
        FetchUtils::shiftOrRotate(pc, m128, m128, 2);        //   m0[15:0] = [??, m[15:2]]
      compOpPart()->nextPartialPixel();                      //   <CompOpPart::nextPartialPixel>
      dPix.resetAllExceptTypeAndName();
      pc->j(L_VTail_Cont, sub_nz(i, 1));                     //   if (--i) goto L_VTail_Cont;
...
void FillAnalyticPart::calcMasksFromCells(const Vec& msk_, const Vec& cov, const Vec& fillRuleMask, const Vec& globalAlpha) noexcept {
  ...
  pc->v_sub_i32(msk, msk, pc->simdConst(&ct.i_0000020000000200, Bcst::k32, msk));
  ...
  if (pc->hasSSE4_1()) {
    ...
    // us a register.
    pc->v_min_u32(msk, msk, pc->simdConst(&ct.i_0000020000000200, Bcst::kNA, msk));
    ...
    switch (coverageFormat) {
      ...
      case PixelCoverageFormat::kPacked: {
        ...
        for (uint32_t i = 0; i < nRegs; i++) {
          Vec v = msk[i].v128();
          pc->v_swizzlev_u8(v, v, pc->simdConst(&pc->ct.pshufb_xxxxxxxxx3x2x1x0_to_3333222211110000, Bcst::kNA, v));
        }
        return;
      }
    ...
    switch (coverageFormat) {
      ...
      case PixelCoverageFormat::kUnpacked: {
        ...
        if (msk[0].isVec256()) {
          ...
          pc->v_swizzle_u64x4(msk[0], msk[0], swizzle(2, 0, 2, 0));
        }
