...
void FetchContext::_initFetchRegs() noexcept {
  ...
  switch (_fetchMode) {
    ...
    case FetchMode::kRGBA32FromA8_PC: {
      ...
      p128VecCount = blMax<uint32_t>(vec128RegCountFromBytes(fullByteCount) >> uint32_t(_pc->simdWidth()), 1u);
...
void FetchContext::fetchPixel(const Mem& src) noexcept {
  ...
  // Maybe unused, because not all backends need it.
  blUnused(pixelCount);
  ...
  switch (_fetchMode) {
    ...
    case FetchMode::kRGBA32FromA8_UC: {
      ...
      _pc->ror(_aAcc, _aAcc, packed ? 8u : 16u);
      _aAccIndex++;

      uint32_t accBytes = packed ? _aAccIndex : _aAccIndex * 2u;
      if (accBytes == accByteSize || _pixelIndex + 1u == pixelCount) {
  ...
  switch (_widening256Op) {
    ...
    case WideningOp::kRepeat: {
      if (_widening512Op == WideningOp::kUnpack) {
        ...
        _pc->v_swizzlev_u8(_p512[index], _p512[index], _pc->simdConst(&_pc->ct.pshufb_xxx3xxx2xxx1xxx0_to_z3z3z2z2z1z1z0z0, Bcst::kNA, _p512[index]));
      }
      else if (_widening512Op == WideningOp::kRepeat) {
        _pc->v_cvt_u8_to_u32(_p512[index], _p128[index]);
        _pc->v_swizzlev_u8(_p512[index], _p512[index], _pc->simdConst(&_pc->ct.pshufb_xxx3xxx2xxx1xxx0_to_3333222211110000, Bcst::kNA, _p512[index]));
      }
      else {
        _pc->v_cvt_u8_lo_to_u16(_p256[index], _p128[index]);
    ...
    case WideningOp::kRepeat8xA8ToRGBA32_UC_AVX512: {
      ...
      if (_pc->hasAVX512_VBMI()) {
        _pc->cc->emit(x86::Inst::kIdVpermb, _p512[index], _p128[index].zmm(), _pc->simdConst(&_pc->ct.vpermb_a8_to_rgba32_uc, Bcst::kNA_Unique, _p512[index]));
      }
      else {
...
void gatherPixels(PipeCompiler* pc, Pixel& p, PixelCount n, FormatExt format, PixelFlags flags, const Mem& src, const Vec& idx, uint32_t shift, IndexLayout indexLayout, InterleaveCallback cb, void* cbData) noexcept {
  Mem mem(src);

#if defined(BL_JIT_ARCH_X86)
  bool forcedGather = pc->hasAVX512() && n > 8u;

  if (pc->hasOptFlag(PipeOptFlags::kFastGather) || forcedGather) {
