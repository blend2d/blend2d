...
public:
  typedef BLResult (BL_CDECL* WorkFunc)(BLRasterFiller* ctx, BLRasterWorkerContext* workerCtx, const BLRasterFetchData* fetchData) BL_NOEXCEPT;
  ...
  BL_INLINE void initBoxAA8bpc(uint32_t alpha, int x0, int y0, int x1, int y1) noexcept {
    ...
    fillSignature.addFillType(fillData.initBoxAA8bpc(alpha, uint32_t(x0), uint32_t(y0), uint32_t(x1), uint32_t(y1)));
  }
  ...
  BL_INLINE BLResult doWork(BLRasterWorkerContext* workerCtx, const BLRasterFetchData* fetchData) noexcept {
    return workFunc(this, workerCtx, fetchData);
  }

  static BLResult BL_CDECL fillRectImpl(BLRasterFiller* ctx, BLRasterWorkerContext* workerCtx, const BLRasterFetchData* fetchData) noexcept;
  static BLResult BL_CDECL fillAnalyticImpl(BLRasterFiller* ctx, BLRasterWorkerContext* workerCtx, const BLRasterFetchData* fetchData) noexcept;
};
