...
bool blThreadEventIsSignaled(const BLThreadEvent* self) noexcept {
  ...
  BLThreadEventPosixImpl* impl = (BLThreadEventPosixImpl*)self->handle;
  BLLockGuard<BLMutex> guard(impl->mutex);
...
BLResult blThreadEventTimedWait(BLThreadEvent* self, uint64_t microseconds) noexcept {
  ...
  while (!impl->signaled)
    if (impl->cond.waitUntil(impl->mutex, &absTime) == BL_ERROR_TIMED_OUT)
...
static BL_INLINE void blThreadEntryPoint(BLInternalThread* thread) noexcept {
  for (;;) {
    ...
    if (!blAtomicCompareExchange(&thread->internalStatus, &value, uint32_t(BL_THREAD_STATUS_RUNNING)) && !workFunc)
      break;
    ...
    bool res = !blAtomicCompareExchange(&thread->internalStatus, &value, uint32_t(BL_THREAD_STATUS_IDLE));
